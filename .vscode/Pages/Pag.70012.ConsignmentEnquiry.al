page 70012 "Consignment Enquiry"
{
    ...
    layout
    {
        area(Content)
        {
            ...
            repeater(Details)
            {
                ...
                field("Total MDR Amount"; Rec."Total MDR Amount") { }
    ...
    actions
    {
        area(Processing)
        {
            ...
            action("Calculate MDR")
            {
                ...
                begin
                    PostMDR(Rec."Start Date", Rec."End Date", rec."Vendor No.");
                end;
            }
            action("Calculate MGP")
            {
                ...
                begin
                    PostMGP(Rec."Start Date", Rec."End Date", rec."Vendor No.");
                end;
            }
        }
        area(Navigation)
    ...
    procedure PostMDR(salesDate: Date; endSalesDate: date; vendor: code[50])
    var
    ...
    begin
        ...
        ConsignEntries.setrange("Vendor No.", vendor);
        ...
        if ConsignEntries.FindSet() then begin
            repeat
                if (ConsignEntries."Start Date" >= salesDate) and (ConsignEntries."End Date" <= endSalesDate) then begin
                    ...
                    //MDR Amt
                    TotalMDRAmt += ConsignEntries."Total MDR Amount";
                    //MGP
                    BillingTotalProfit += ConsignEntries."Billing - Total Cost";
                    if ConsignEntries."Expected Gross Profit" <> 0 then
                        ExpectedGrossProfit := ConsignEntries."Expected Gross Profit";
                end;
        ...
        IF ABS(MDRAmt) <> 0 then begin
            ...
            LRecSH."Posting Date" := endSalesDate;
    ...
    procedure PostMGP(salesDate: Date; endSalesDate: date; vendor: code[50])
    var
        LRecCP: Record "Consignment Process Log";
        NextEntryNo: Integer;
        NextLineNo: Integer;
    ...
    begin
        ...
        ConsignAmt := Round(BillingTotalProfit);
        BillableMPGAmt := ExpectedGrossProfit - ConsignAmt;
        IF ABS(BillableMPGAmt) > 0 then begin
            lrecsh.Invoice := true;
